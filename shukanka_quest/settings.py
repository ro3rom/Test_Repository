...
# 開発環境用に静的ファイルのディレクトリを指定する場合
STATICFILES_DIRS = [
    BASE_DIR / "static",  # プロジェクトのルートにある "static"
]
...
TEMPLATES = [
    {
        ...
        'DIRS': [BASE_DIR / 'templates'],  # ここにテンプレートのパスを指定
        'APP_DIRS': True,
        'OPTIONS': {
            'context_processors': [
...
TEMPLATES = [
    {
        'BACKEND': 'django.template.backends.django.DjangoTemplates',
        'DIRS': [
            BASE_DIR / 'shukanka_quest' / 'templates',  # shukanka_quest/templates ディレクトリを指定
        ],
        'APP_DIRS': True,
        'OPTIONS': {
            'context_processors': [
...
LOGIN_URL = 'login'  # ログインしていないユーザーはこのURLにリダイレクトされる